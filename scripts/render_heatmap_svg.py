...
terminal-card contribution heatmap SVG: the classic 53-week x 7-day grid of
rounded boxes, revealed once with a diagonal cascade (CSS keyframes, plays
on load then freezes), a Less->More legend, and a real stats footer.
...
USER_HANDLE = os.environ.get("GH_PROFILE_USER", "Tanishq321").lower()
...
def level_from_count(count, level_hint):
    # trust the scraped 0-4 bucket when we have it; otherwise bucket the count
    if level_hint is not None:
        return min(level_hint, 4)
    if count == 0:
        return 0
    if count <= 5:
        return 1
...
def build_grid(days):
    ...
    for d in days:
        ...
        while len(col) < weekday:
            col.append(None)
        lvl = level_from_count(d["count"], d.get("level"))
