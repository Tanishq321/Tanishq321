#!/usr/bin/env python3
"""
Pull real daily contribution counts from GitHub's public, unauthenticated
contributions endpoint (the same fragment that renders on the profile page)
and write data/contributions.json with the raw days plus a few derived
stats (current streak, longest streak, best day, monthly totals).

No token needed. Meant to be re-run on a schedule (see
.github/workflows/update-profile-art.yml) so the numbers stay fresh.
"""
import datetime
import json
import os
...
USERNAME = os.environ.get("GH_PROFILE_USER", "Tanishq321")
...
def fetch_days():
    ...
    for td in cells:
        ...
        if not date:
            continue
        # GitHub currently exposes a 0-4 bucket directly on the cell, which
        # is more stable than parsing the tooltip sentence -- fall back to
        # the tooltip text (for an exact count) when it's present.
        ...
        count = int(m.group(1).replace(",", "")) if m else level
